...
ifeq ($(OS),Linux)
    ...
    EXT =
    ARCHIVE_CMD = tar -czf battleship-linux.tar.gz $(OUT) README.md resources
else ifeq ($(OS),Darwin)  # Darwin is the macOS kernel name
    LDFLAGS = $(LDFLAGS_MACOS)
    EXT =
    ARCHIVE_CMD = tar -czf battleship-macos.tar.gz $(OUT) README.md resources
else
    LDFLAGS = $(LDFLAGS_WINDOWS)
    EXT = .exe
    ARCHIVE_CMD = zip battleship-windows.zip $(OUT) README.md resources
endif
...
# Clean
clean:
	rm -f battleship battleship.exe battleship-linux.tar.gz battleship-windows.zip battleship-macos.tar.gz
